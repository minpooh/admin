...
const PRODUCTION_STATUS = ['제작완료', '미제작', '제작중'];
...
function matchProductionStatus(order: OrderItem, production: string): boolean {
  switch (production) {
    case '미제작':
      return order.progress === '제작전';
    case '제작중':
      return order.progress === '제작중';
    case '제작완료':
      return (
        order.progress === '제작완료' ||
        order.progress === '고객자료완료' ||
        order.progress === 'dvd확정미출고' ||
        order.progress === 'dvd미확정' ||
        order.progress === 'usb확정미출고' ||
        order.progress === 'usb미확정'
      );
    default:
      return true;
  }
...
function applyFilters(orders: OrderItem[], applied: AppliedSearch | null): OrderItem[] {
  ...
  return orders.filter((order) => {
    ...
    if (useCustomRange) {
      if (!isInCustomDateRange(order.firstProductionDate, applied.startDate, applied.endDate)) return false;
    } else if (!isInDateRange(order.firstProductionDate, applied.dateRange)) {
      return false;
    }
...
export default function OrderVideoTestPage() {
  const [filterExpanded, setFilterExpanded] = useState(false);
  // 최초제작일을 아직 선택하지 않았을 때는 어떤 프리셋도 활성화하지 않음.
  ...
  const appliedChips: Array<{ key: AppliedChipKey; label: string }> = (() => {
    ...
    if (appliedSearch.startDate || appliedSearch.endDate) {
      ...
      chips.push({
        key: 'date',
        label: `최초제작일: ${start}${start && end ? ' ~ ' : ''}${end}`,
      });
    } else if (appliedSearch.dateRange) {
      chips.push({ key: 'date', label: `최초제작일: ${appliedSearch.dateRange}` });
    }
