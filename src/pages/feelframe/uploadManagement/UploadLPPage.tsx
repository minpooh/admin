...
import {
  MOCK_FEELFRAME_UPLOAD_LP_LIST,
  type FeelframeUploadLPMemoEntry,
  type FeelframeUploadLPProgress,
  type FeelframeUploadLPRow,
} from './mock/uploadLP.mock';
...
function applyFilters(rows: FeelframeUploadLPRow[], search: AppliedSearch | null) {
...
function getProgressClassName(status: FeelframeUploadLPProgress) {
  if (status === '업로드전') return 'progress-status progress-status--warning';
  if (status === '업로드완료') return 'progress-status progress-status--blue';
  return 'progress-status progress-status--secondary';
}

function getCorrectionRequestedLabel(request: string) {
  return request.trim() === '없음' ? 'X' : 'O';
}

function getCorrectionIntensityLabel(request: string, intensity: string) {
  if (getCorrectionRequestedLabel(request) === 'X') return '-';
  if (intensity === '강함' || intensity === '강') return '강';
  if (intensity === '보통' || intensity === '중') return '중';
  if (intensity === '약함' || intensity === '약') return '약';
  return '디자이너 임의';
}
...
function getUploadOrderDetailPreviewItems(row: FeelframeUploadLPRow): UploadOrderDetailPreviewItem[] {
  ...
  return Array.from({ length: totalCount }, (_, idx) => ({
    ...
    optionLabel: `${row.shippingCarrierName} / ${idx + 1}번째 상품`,
  }));
}

export default function FeelframeUploadLPPage() {
  const [rows, setRows] = useState<FeelframeUploadLPRow[]>(() => [...MOCK_FEELFRAME_UPLOAD_LP_LIST]);
  ...
  const addMemo = (rowId: string) => {
    ...
    const nextMemo: FeelframeUploadLPMemoEntry = {
      id: `memo-${Date.now()}`,
  ...
  return (
    <div className="admin-list-page">
      <h1 className="page-title">LP 업로드</h1>
      ...
      <section className="admin-list-box admin-list-box--table" aria-label="LP 업로드 목록">
        ...
        <div className="admin-table-wrap">
          <table className="admin-table admin-table--feelframe-upload-lp">
            <thead>
              <tr>
                ...
                <th>예식예정일</th>
                <th>고객정보</th>
                <th className="col-center">보정요청</th>
            ...
            <tbody>
              {paginatedRows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="col-center">{getCorrectionRequestedLabel(row.correctionRequest)}</td>
                  <td className="col-center">
                    {(() => {
                      const intensityLabel = getCorrectionIntensityLabel(row.correctionRequest, row.correctionIntensity);
                      if (intensityLabel === '-') return '-';
                  ...
                  <td className="col-center">
                    ...
                    >
                      <button
                        type="button"
                        className={`row-btn ${row.adminPreviewImages.length > 0 ? 'row-btn--primary' : 'row-btn--default'}`}
