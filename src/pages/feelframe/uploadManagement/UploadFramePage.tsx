...
function applyFilters(rows: FeelframeUploadFrameRow[], search: AppliedSearch | null) {
...
function getProgressClassName(status: FeelframeUploadFrameProgress) {
  ...
  if (status === '고객업로드' || status === '상품준비중') return 'progress-status progress-status--warning';
...
function getUploadOrderDetailPreviewItems(row: FeelframeUploadFrameRow): UploadOrderDetailPreviewItem[] {
...
export default function FeelframeUploadFramePage() {
  ...
  const addMemo = (rowId: string) => {
    ...
    const nextMemo: FeelframeUploadFrameMemoEntry = {
      id: `memo-${Date.now()}`,
  ...
  return (
    <div className="admin-list-page">
      <h1 className="page-title">액자 업로드</h1>
      ...
      <section className="admin-list-box admin-list-box--table" aria-label="액자 업로드 목록">
        ...
        <div className="admin-table-wrap">
          <table className="admin-table admin-table--feelframe-upload-frame">
            <thead>
              <tr>
