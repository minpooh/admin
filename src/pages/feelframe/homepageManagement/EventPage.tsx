...
function buildNewEventDraftRow(): EventRow {
...
export default function EventPage() {
  ...
  const [rows, setRows] = useState<EventRow[]>(() => [...EVENT_ROWS_MOCK]);
  ...
  const handleToggleExposed = (row: EventRow) => {
    setConfirmDialog({
      ...
      onConfirm: () => {
        setRows((prev) =>
          prev.map((item) => (item.id === row.id ? { ...item, exposed: !item.exposed } : item)),
        );
      },
    });
  };

  const handleToggleProgressStatus = (row: EventRow) => {
    setConfirmDialog({
      message: '변경하시겠습니까?',
      confirmText: '확인',
      cancelText: '취소',
      onConfirm: () => {
        setRows((prev) =>
          prev.map((item) =>
            item.id === row.id
              ? { ...item, progressStatus: item.progressStatus === '진행중' ? '종료' : '진행중' }
              : item,
          ),
        );
      },
  ...
  const handleAddEvent = () => {
    navigate(eventNewPath());
  };

  if (subId) {
    if (subId === EVENT_NEW_SUB_ID) {
      const draftRow = buildNewEventDraftRow();
      return (
        <EventDetailPage
          key={EVENT_NEW_SUB_ID}
          row={draftRow}
          isCreate
          onCancelCreate={() => navigate(eventListPath)}
          onSave={(next) => {
            ...
            setCurrentPage(1);
            navigate(eventDetailPath(id), { replace: true });
          }}
        />
      );
    }
      ...
      return (
        <div className="admin-list-page">
          <div className="admin-detail-header">
            ...
            <h1 className="page-title">이벤트 상세</h1>
          </div>
          <section className="admin-list-box">
            <p className="admin-list-result">이벤트를 찾을 수 없습니다.</p>
          </section>
        </div>
      );
    }
  ...
  return (
    <div className="admin-list-page">
      <h1 className="page-title">이벤트관리</h1>
      ...
      <section className="admin-list-box admin-list-box--table" aria-label="이벤트 목록">
              ...
              {paginatedRows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="col-center">
                    <button
                      type="button"
                      className={`row-btn ${row.progressStatus === '진행중' ? 'row-btn--blue' : 'row-btn--red'}`}
                      onClick={() => handleToggleProgressStatus(row)}
                    >
                      {row.progressStatus}
                    </button>
                  </td>
      ...
      <div className="admin-list-add-row">
        <button type="button" className="admin-list-add-btn" onClick={handleAddEvent} aria-label="이벤트 추가">
