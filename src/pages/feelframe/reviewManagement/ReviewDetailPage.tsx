...
import type {
  FeelframeReviewAttachment,
  FeelframeReviewDetailData,
  FeelframeReviewThreadEntry,
} from './mock/review.mock';
import { getFeelframeReviewDetailById } from './mock/review.mock';
import { reviewListPath } from './reviewPaths';
...
function sortThreadByDate(entries: FeelframeReviewThreadEntry[]) {
...
function newThreadId(): string {
  return `fr-th-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
}
...
function downloadReviewAttachment(file: FeelframeReviewAttachment) {
...
function cloneDetail(subId: string | undefined): FeelframeReviewDetailData | undefined {
  if (!subId) return undefined;
  const next = getFeelframeReviewDetailById(subId);
  return next ? { ...next, thread: [...next.thread], attachments: [...next.attachments] } : undefined;
}

export default function ReviewDetailPage() {
  const { subId } = useParams<{ subId?: string }>();
  const [localDetail, setLocalDetail] = useState<FeelframeReviewDetailData | undefined>(() =>
    cloneDetail(subId),
  );
  ...
  const commitReply = useCallback((html: string, mode: 'new' | 'edit', editId?: string) => {
    setLocalDetail((prev) => {
      if (!prev) return prev;
      let thread: FeelframeReviewThreadEntry[];
      if (mode === 'new') {
        if (prev.thread.length > 0) return prev;
        thread = [
          {
            id: newThreadId(),
            role: 'admin',
    ...
    return (
      <div className="admin-list-page admin-list-page--feelframe-review">
        <div className="admin-detail-header">
          <Link to={reviewListPath} className="admin-detail-back">
            ← 목록
          </Link>
        ...
        <section className="admin-list-box">
          ...
          <p className="admin-detail-notice">
            <Link to={reviewListPath}>목록으로 돌아가기</Link>
          </p>
        </section>
      </div>
    );
  ...
  const showRegisterBtn = replyEditor.status !== 'new' && adminReplies.length === 0;
              ...
              <div className="cell-block">
                <span className="cell-line">
                  <span
                    className={[
                      'admin-status-pill',
                      row.answeredAt !== null ? 'admin-status-pill--답변완료' : 'admin-status-pill--답변전',
                    ].join(' ')}
                  >
                    {row.answeredAt !== null ? '답변완료' : '답변전'}
                  </span>
                </span>
              </div>
            </dd>
                ...
                {row.attachments.map((file) => (
                  <li key={file.id}>
                    <button
                      ...
                      onClick={() => downloadReviewAttachment(file)}
                    >
